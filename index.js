...
function getUsersFromDatabase(array) {
  ...
  const arr = Array.from(array, (user) => ({
    id: user.id,
...
function createArray(...elements) {
  ...
  if (elements.length > 0) {
    ...
    console.error("You have a mistake");
    return [];
  }
  // return elements;
}
...
function findElementIndexes(arr, element) {
  ...
  const el1 = arr.indexOf(element);
  const el2 = arr.lastIndexOf(element);
  const result = [];
  if (el1 > 0) {
    result.push(el1);
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (0 < target <= arr.length) {
    console.error("Неприпустимі індекси");
  }
  const array = arr.copyWithin(target, start, end);
  return array;
}
...
function customEvery(arr, condition) {
  ...
  const every = arr.every(condition);
  return every;
}
...
function customIncludes(arr, element) {
  ...
  const elem = arr.includes(element);
  const filtCount = arr.filter((el) => el === element).length;
